...
import cv2
import numpy as np
from driving_model.waymo.model import DrivingModel  # import your new DrivingModel
from torchvision import transforms

class SteeringModel:
    def __init__(self, model_path="waymo_model.pth"):
        ...
        # Make sure preprocessing matches training
    ...
    def preprocess(self, img, mask=None):
        img = img[90:, :, :]  # crop if needed
        img = cv2.cvtColor(img, cv2.COLOR_BGR2RGB)
        img = self.transform(img)
        return img

    def waypoints_to_steering(self, waypoints):
        # Convert 2D waypoints to steering angle
